...
def differentExponent : ℕ :=
  multiplicity (IsLocalRing.maximalIdeal (𝓞 L)) (differentIdeal (𝓞 K) (𝓞 L))
...
def discriminantExponent : ℕ :=
  multiplicity (IsLocalRing.maximalIdeal (𝓞 K))
    (Ideal.span {Algebra.discr (𝓞 K)
      ⇑(Module.Free.chooseBasis (𝓞 K) (𝓞 L))})
...
instance separable_fractionRing :
  ...
  ext x
  have h := IsFractionRing.algEquiv_commutes (FractionRing.algEquiv (𝓞 K) K)
    (FractionRing.algEquiv (𝓞 L) L) ((FractionRing.algEquiv (𝓞 K) K).symm x)
...
theorem inertiaDeg_ne_zero : inertiaDeg K L ≠ 0 :=
  (Ideal.inertiaDeg_pos (IsLocalRing.maximalIdeal (𝓞 K))
    (IsLocalRing.maximalIdeal (𝓞 L))).ne'
...
theorem relNorm_differentIdeal_eq_span_discr :
    Ideal.relNorm (𝓞 K) (differentIdeal (𝓞 K) (𝓞 L)) =
      Ideal.span {Algebra.discr (𝓞 K) ⇑(Module.Free.chooseBasis (𝓞 K) (𝓞 L))} := by
  classical
  letI : IsFractionRing (𝓞 K) K := RingOfIntegers.instIsFractionRing (K := K)
  letI : IsFractionRing (𝓞 L) L := RingOfIntegers.instIsFractionRing (K := L)
  haveI : Algebra.IsSeparable K L := isSeparable_of_padicExtension p K L
  obtain ⟨θ, hθtop⟩ : ∃ θ : 𝓞 L, Algebra.adjoin (𝓞 K) ({θ} : Set (𝓞 L)) = ⊤ :=
    Monogenicity.mono_exists_primitive
  have hθ_int : IsIntegral (𝓞 K) θ := Algebra.IsIntegral.isIntegral θ
  let e : ↥(Algebra.adjoin (𝓞 K) ({θ} : Set (𝓞 L))) ≃ₐ[𝓞 K] (𝓞 L) :=
    (Subalgebra.equivOfEq _ _ hθtop).trans Subalgebra.topEquiv
  let pb : PowerBasis (𝓞 K) (𝓞 L) := (Algebra.adjoin.powerBasis' hθ_int).map e
  have hpb_gen : pb.gen = θ := by
    show e (Algebra.adjoin.powerBasis' hθ_int).gen = θ
    rw [Algebra.adjoin.powerBasis'_gen]; rfl
  have hθL_int : IsIntegral K (algebraMap (𝓞 L) L θ) := Algebra.IsIntegral.isIntegral _
  have hmin : minpoly K (algebraMap (𝓞 L) L θ) =
      Polynomial.map (algebraMap (𝓞 K) K) (minpoly (𝓞 K) θ) :=
    minpoly.isIntegrallyClosed_eq_field_fractions K L hθ_int
  have hfdeg : (minpoly (𝓞 K) θ).natDegree = Module.finrank K L := by
    have h1 : pb.dim = (minpoly (𝓞 K) θ).natDegree := by
      show (Algebra.adjoin.powerBasis' hθ_int).dim = _
      rw [Algebra.adjoin.powerBasis'_dim]
    have h2 : Module.finrank (𝓞 K) (𝓞 L) = pb.dim := pb.finrank
    rw [← h1, ← h2]; exact (free_finrank K L).2
  have hdegK : (minpoly K (algebraMap (𝓞 L) L θ)).natDegree =
      (minpoly (𝓞 K) θ).natDegree := by
    rw [hmin, Polynomial.natDegree_map_eq_of_injective (IsFractionRing.injective (𝓞 K) K)]
  have hxK : Algebra.adjoin K {algebraMap (𝓞 L) L θ} = ⊤ := by
    have hsub : (Algebra.adjoin K {algebraMap (𝓞 L) L θ}).toSubmodule = ⊤ := by
      apply Submodule.eq_top_of_finrank_eq
      show Module.finrank K ↥(Algebra.adjoin K {algebraMap (𝓞 L) L θ}) = Module.finrank K L
  ...
  have hcond : conductor (𝓞 K) θ = ⊤ := by
    rw [Ideal.eq_top_iff_one, mem_conductor_iff]
    intro b
    rw [one_mul, hθtop]
    exact Algebra.mem_top
  have hdiff : differentIdeal (𝓞 K) (𝓞 L) =
      Ideal.span {Polynomial.aeval θ (Polynomial.derivative (minpoly (𝓞 K) θ))} := by
    have h := conductor_mul_differentIdeal (𝓞 K) K L θ hxK
    rwa [hcond, Ideal.top_mul] at h
  have hG2 := intNorm_deriv_minpoly_associated_discr (K := K) (L := L) pb
  rw [hpb_gen] at hG2
  have hG1 := Algebra.discr_associated_of_basis (R := 𝓞 K)
    pb.basis (Module.Free.chooseBasis (𝓞 K) (𝓞 L))
...
theorem isUnramifiedAt_maximalIdeal_iff :
    Algebra.IsUnramifiedAt (𝓞 K) (IsLocalRing.maximalIdeal (𝓞 L)) ↔ IsUnramified K L := by
  have hp : maximalIdeal (𝓞 L) ≠ ⊥ := IsDiscreteValuationRing.not_a_field (𝓞 L)
  have hover : Ideal.under (𝓞 K) (maximalIdeal (𝓞 L)) = maximalIdeal (𝓞 K) :=
    Ideal.LiesOver.over.symm
  letI := Localization.AtPrime.algebraOfLiesOver
    (Ideal.under (𝓞 K) (maximalIdeal (𝓞 L))) (maximalIdeal (𝓞 L))
  haveI : Finite (𝓞 K ⧸ Ideal.under (𝓞 K) (maximalIdeal (𝓞 L))) := by
    rw [hover]
    exact inferInstanceAs (Finite (IsLocalRing.ResidueField (𝓞 K)))
  haveI : Finite ((Ideal.under (𝓞 K) (maximalIdeal (𝓞 L))).ResidueField) := inferInstance
  haveI : Finite (𝓞 L ⧸ maximalIdeal (𝓞 L)) := inferInstanceAs
    (Finite (IsLocalRing.ResidueField (𝓞 L)))
  haveI : Finite ((maximalIdeal (𝓞 L)).ResidueField) := inferInstance
  haveI : PerfectField ((Ideal.under (𝓞 K) (maximalIdeal (𝓞 L))).ResidueField) := inferInstance
  haveI : Algebra.IsAlgebraic ((Ideal.under (𝓞 K) (maximalIdeal (𝓞 L))).ResidueField)
      ((maximalIdeal (𝓞 L)).ResidueField) := inferInstance
  rw [Algebra.isUnramifiedAt_iff_map_eq (𝓞 K)
      (Ideal.under (𝓞 K) (maximalIdeal (𝓞 L))) (maximalIdeal (𝓞 L)),
    and_iff_right (Algebra.IsAlgebraic.isSeparable_of_perfectField :
      Algebra.IsSeparable ((Ideal.under (𝓞 K) (maximalIdeal (𝓞 L))).ResidueField)
        ((maximalIdeal (𝓞 L)).ResidueField)),
    ← Ideal.IsDedekindDomain.ramificationIdx_eq_one_iff hp Ideal.map_comap_le]
  show Ideal.ramificationIdx (Ideal.under (𝓞 K) (maximalIdeal (𝓞 L)))
      (maximalIdeal (𝓞 L)) = 1 ↔ IsUnramified K L
  rw [hover]
  exact Iff.rfl
...
theorem discExponent_eq_inertiaDeg_mul_differentExponent :
  ...
  have hdpow : differentIdeal (𝓞 K) (𝓞 L) =
      (IsLocalRing.maximalIdeal (𝓞 L)) ^ (differentExponent K L) :=
    eq_maximalIdeal_pow_multiplicity hd
  have hrelmax : Ideal.relNorm (𝓞 K) (IsLocalRing.maximalIdeal (𝓞 L)) =
      (IsLocalRing.maximalIdeal (𝓞 K)) ^ (inertiaDeg K L) :=
    Ideal.relNorm_eq_pow_of_isMaximal _ _
  have key : Ideal.span {Algebra.discr (𝓞 K) ⇑(Module.Free.chooseBasis (𝓞 K) (𝓞 L))}
      = (IsLocalRing.maximalIdeal (𝓞 K)) ^ (inertiaDeg K L * differentExponent K L) := by
    rw [← relNorm_differentIdeal_eq_span_discr K L, hdpow, map_pow, hrelmax, ← pow_mul]
  show multiplicity (IsLocalRing.maximalIdeal (𝓞 K))
      (Ideal.span {Algebra.discr (𝓞 K) ⇑(Module.Free.chooseBasis (𝓞 K) (𝓞 L))}) = _
...
lemma ramificationIdx_sub_one_le_differentExponent :
  ...
  have hd : differentIdeal (𝓞 K) (𝓞 L) ≠ ⊥ := differentIdeal_ne_bot
  have hfin : FiniteMultiplicity (maximalIdeal (𝓞 L)) (differentIdeal (𝓞 K) (𝓞 L)) :=
    FiniteMultiplicity.of_not_isUnit
      (Ideal.isUnit_iff.not.mpr (IsLocalRing.maximalIdeal.isMaximal (𝓞 L)).ne_top)
      (by rw [Ideal.zero_eq_bot]; exact hd)
  have hdvd : (maximalIdeal (𝓞 L)) ^ (ramificationIdx K L) ∣
      (maximalIdeal (𝓞 K)).map (algebraMap (𝓞 K) (𝓞 L)) :=
    Ideal.dvd_iff_le.mpr Ideal.le_pow_ramificationIdx
  have hdvd' : (maximalIdeal (𝓞 L)) ^ (ramificationIdx K L - 1) ∣
      differentIdeal (𝓞 K) (𝓞 L) :=
    pow_sub_one_dvd_differentIdeal (P := maximalIdeal (𝓞 L)) (e := ramificationIdx K L)
      (hp := hmK) (hP := hdvd)
...
lemma intTrace_mem_maximalIdeal_of_dvd_ramificationIdx
    ...
    ∀ b : 𝓞 L, Algebra.intTrace (𝓞 K) (𝓞 L) b ∈ IsLocalRing.maximalIdeal (𝓞 K) := by
  ...
  haveI hene2 : NeZero (Ideal.ramificationIdx (R := 𝓞 K) (S := 𝓞 L)
    (maximalIdeal (𝓞 K)) (maximalIdeal (𝓞 L))) := hene
  haveI hmaxK : (maximalIdeal (𝓞 K)).IsMaximal := IsLocalRing.maximalIdeal.isMaximal (𝓞 K)
  haveI hmaxL : (maximalIdeal (𝓞 L)).IsMaximal := IsLocalRing.maximalIdeal.isMaximal (𝓞 L)
  letI : Field (𝓞 K ⧸ maximalIdeal (𝓞 K)) := Ideal.Quotient.field _
  letI : Field (𝓞 L ⧸ maximalIdeal (𝓞 L)) := Ideal.Quotient.field _
  haveI : Finite (𝓞 K ⧸ maximalIdeal (𝓞 K)) :=
    inferInstanceAs (Finite (IsLocalRing.ResidueField (𝓞 K)))
  haveI : Finite (𝓞 L ⧸ maximalIdeal (𝓞 L)) :=
    inferInstanceAs (Finite (IsLocalRing.ResidueField (𝓞 L)))
  letI hAlg : Algebra (𝓞 K ⧸ maximalIdeal (𝓞 K)) (𝓞 L ⧸ maximalIdeal (𝓞 L)) :=
    Ideal.Quotient.algebraQuotientOfRamificationIdxNeZero
      (maximalIdeal (𝓞 K)) (maximalIdeal (𝓞 L))
  letI : Module (𝓞 K ⧸ maximalIdeal (𝓞 K)) (𝓞 L ⧸ maximalIdeal (𝓞 L)) := hAlg.toModule
  haveI : Module.Finite (𝓞 K ⧸ maximalIdeal (𝓞 K)) (𝓞 L ⧸ maximalIdeal (𝓞 L)) :=
    Module.Finite.of_finite
  haveI : Algebra.IsAlgebraic (𝓞 K ⧸ maximalIdeal (𝓞 K)) (𝓞 L ⧸ maximalIdeal (𝓞 L)) :=
    Algebra.IsAlgebraic.of_finite _ _
  haveI : PerfectField (𝓞 K ⧸ maximalIdeal (𝓞 K)) := inferInstance
  haveI hsep : Algebra.IsSeparable (𝓞 K ⧸ maximalIdeal (𝓞 K)) (𝓞 L ⧸ maximalIdeal (𝓞 L)) :=
    Algebra.IsAlgebraic.isSeparable_of_perfectField
  haveI hcharK : CharP (𝓞 K ⧸ maximalIdeal (𝓞 K)) p := by
    refine (CharP.charP_iff_prime_eq_zero (Fact.out (p := p.Prime))).mpr ?_
    rw [← map_natCast (Ideal.Quotient.mk (maximalIdeal (𝓞 K))) p,
      Ideal.Quotient.eq_zero_iff_mem, IsLocalRing.mem_maximalIdeal, mem_nonunits_iff]
    intro hu
    rw [show (p : 𝓞 K) = algebraMap ℤ_[p] (𝓞 K) (p : ℤ_[p]) from (map_natCast _ p).symm] at hu
  ...
  have hcore := fun x => TraceFiltration.intTrace_residue_scaling
    (p := maximalIdeal (𝓞 K)) (P := maximalIdeal (𝓞 L)) x hP0 hPe
  intro b
  rw [← Ideal.Quotient.eq_zero_iff_mem, hcore b, nsmul_eq_mul]
  apply mul_eq_zero_of_left
  rw [CharP.cast_eq_zero_iff (𝓞 K ⧸ maximalIdeal (𝓞 K)) p]
  exact hdvd
...
lemma exists_intTrace_not_mem_maximalIdeal_of_not_dvd
    ...
    ∃ x : 𝓞 L, Algebra.intTrace (𝓞 K) (𝓞 L) x ∉ IsLocalRing.maximalIdeal (𝓞 K) := by
  have hP0 : maximalIdeal (𝓞 L) ≠ ⊥ := IsDiscreteValuationRing.not_a_field (𝓞 L)
  have hPe := map_maximalIdeal_eq_pow_ramificationIdx K L
  haveI hene : NeZero (ramificationIdx K L) := ramificationIdx_ne_zero K L
  haveI hene2 : NeZero (Ideal.ramificationIdx (R := 𝓞 K) (S := 𝓞 L)
  ...
  obtain ⟨y, hy⟩ := Algebra.trace_surjective
    (K := 𝓞 K ⧸ maximalIdeal (𝓞 K)) (L := 𝓞 L ⧸ maximalIdeal (𝓞 L)) 1
  obtain ⟨x, hx⟩ := Ideal.Quotient.mk_surjective y
  have htr : (Ideal.Quotient.mk (maximalIdeal (𝓞 K)))
      (Algebra.intTrace (𝓞 K) (𝓞 L) x) ≠ 0 := by
    rw [hcore x, hx, hy, nsmul_eq_mul, mul_one, ne_eq,
      CharP.cast_eq_zero_iff (𝓞 K ⧸ maximalIdeal (𝓞 K)) p]
    exact htame
...
lemma ramificationIdx_le_differentExponent_of_dvd
  ...
  have hfin : FiniteMultiplicity (maximalIdeal (𝓞 L)) (differentIdeal (𝓞 K) (𝓞 L)) :=
    FiniteMultiplicity.of_not_isUnit
      ...
      (by rw [Ideal.zero_eq_bot]; exact hd)
  have hPe := map_maximalIdeal_eq_pow_ramificationIdx K L
  have hmem : ∀ b : 𝓞 L, Algebra.intTrace (𝓞 K) (𝓞 L) b ∈ maximalIdeal (𝓞 K) :=
    intTrace_mem_maximalIdeal_of_dvd_ramificationIdx K L hdvd
  obtain ⟨ϖ, hϖ⟩ := IsDiscreteValuationRing.exists_irreducible (𝓞 K)
  have hspan : maximalIdeal (𝓞 K) = Ideal.span {ϖ} := hϖ.maximalIdeal_eq
  have hϖne : (algebraMap (𝓞 K) K) ϖ ≠ 0 :=
    (map_ne_zero_iff _ (IsFractionRing.injective (𝓞 K) K)).mpr hϖ.ne_zero
  have hIne : (↑(maximalIdeal (𝓞 L) ^ ramificationIdx K L) : FractionalIdeal (𝓞 L)⁰ L) ≠ 0 :=
    FractionalIdeal.coeIdeal_ne_zero.mpr (pow_ne_zero _ hP0)
  have hle_id : differentIdeal (𝓞 K) (𝓞 L) ≤ maximalIdeal (𝓞 L) ^ ramificationIdx K L := by
    ...
    have hyϖmem : algebraMap (𝓞 L) L (algebraMap (𝓞 K) (𝓞 L) ϖ) ∈
        (↑(maximalIdeal (𝓞 L) ^ ramificationIdx K L) : FractionalIdeal (𝓞 L)⁰ L) := by
      rw [FractionalIdeal.mem_coeIdeal]
      refine ⟨algebraMap (𝓞 K) (𝓞 L) ϖ, ?_, rfl⟩
      rw [← hPe]
    ...
    have htower : algebraMap (𝓞 L) L (algebraMap (𝓞 K) (𝓞 L) ϖ)
        = algebraMap K L (algebraMap (𝓞 K) K ϖ) := by
    ...
    have hkey : (algebraMap (𝓞 K) K ϖ) * Algebra.trace K L z
        = (algebraMap (𝓞 K) K ϖ) * algebraMap (𝓞 K) K d := by
      have h1 : Algebra.trace K L (z * algebraMap K L (algebraMap (𝓞 K) K ϖ))
          = (algebraMap (𝓞 K) K ϖ) * Algebra.trace K L z := by
        rw [mul_comm z, ← Algebra.smul_def, map_smul, smul_eq_mul]
      have h2 : Algebra.trace K L (z * algebraMap K L (algebraMap (𝓞 K) K ϖ))
          = algebraMap (𝓞 K) K (Algebra.intTrace (𝓞 K) (𝓞 L) c₀) := by
...
lemma differentExponent_lt_ramificationIdx_of_not_dvd
  ...
  have hPmul : (maximalIdeal (𝓞 L)) ^ (ramificationIdx K L) * (⊤ : Ideal (𝓞 L))
      = Ideal.map (algebraMap (𝓞 K) (𝓞 L)) (maximalIdeal (𝓞 K)) := by
    rw [Ideal.mul_top]; exact hPe.symm
  have hndvd : ¬ (maximalIdeal (𝓞 L)) ^ (ramificationIdx K L) ∣
      differentIdeal (𝓞 K) (𝓞 L) :=
    not_dvd_differentIdeal_of_intTrace_not_mem
      (P := maximalIdeal (𝓞 L) ^ ramificationIdx K L) (Q := ⊤)
...
theorem discExponent_eq_zero_iff_unramified :
  ...
  show multiplicity (IsLocalRing.maximalIdeal (𝓞 L)) (differentIdeal (𝓞 K) (𝓞 L)) = 0 ↔ _
